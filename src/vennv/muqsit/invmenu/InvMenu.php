...
namespace vennv\muqsit\invmenu;

use Closure;
use LogicException;
use vennv\muqsit\invmenu\inventory\SharedInvMenuSynchronizer;
use vennv\muqsit\invmenu\session\InvMenuInfo;
use vennv\muqsit\invmenu\session\network\PlayerNetwork;
use vennv\muqsit\invmenu\transaction\DeterministicInvMenuTransaction;
use vennv\muqsit\invmenu\transaction\InvMenuTransaction;
use vennv\muqsit\invmenu\transaction\InvMenuTransactionResult;
use vennv\muqsit\invmenu\transaction\SimpleInvMenuTransaction;
...
class InvMenu implements InvMenuTypeIds{
	...
	public function handleInventoryTransaction(Player $player, Item $out, Item $in, SlotChangeAction $action, InventoryTransaction $transaction) : InvMenuTransactionResult{
		...
		if ($this->listener !== null) {
            return ($this->listener)($inv_menu_txn);
        } else {
            return $inv_menu_txn->continue();
        }
	}
